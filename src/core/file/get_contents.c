...
void file__get_contents(i_node_data_t *i_node_data, uint8_t* contents) {
    ...
    for (; i < block_info.number_of_blocks - 1; ++i) {
        block_data_t block_data = file__get_block_data(i_node_data, i);
        memcpy(contents + i * 32, &block_data, 32);
    }

    block_data_t block_data = file__get_block_data(i_node_data, block_info.number_of_blocks - 1);
...
void file_id__get_contents(uint8_t i_node_id, uint8_t* contents) {
    ...
    file__get_contents(&i_node_data, contents);
}
