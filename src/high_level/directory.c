...
void high_level__dir_create(char * name, int * error, char * error_description) {
    ...
    if (*error != 0) {
        strcpy(error_description, "File limit");
        return;
    }

    dir_data_id__write_file_in_subdirs(ROOT_DIR_ID, name2, new_dir_i_node_id, error);
    if (*error != 0) {
        dir_data_id__delete(new_dir_i_node_id);
        strcpy(error_description, "No such path");
        return;
    }
}
